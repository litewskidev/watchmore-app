...
import { image185Path, imagePath } from '../../utils/tmdbConfig.js';
          ...
          <div className='collection__card__box' key={part.id}>
            <div className='collection__card__poster'>
              {(window.matchMedia('(max-width: 540px)').matches) ? (
                ...
                <LazyLoadImage src={imagePath + part.poster_path} effect='blur' onClick={() => navigate(`/movie/${part.id}`)} alt={part.title}/>
              )}
            </div>
          </div>
          ) : (null)
